...
contract User{
	...
	function User(address _action, address _admin) public{
		owner = msg.sender;
	...
	function changeActionContract(address _action) onlyBy(actionContract) public{
		actionContract = _action;
	}

	function changeAdminContract(address _admin) onlyBy(adminContract) public{
		adminContract = _admin;
	}


	function changeBlackList(address _add, bool _state) onlyBy(owner) public {
		blacklist[_add]  = _state;
	}

	function changeAccessPermissionState(address _ad, uint _state) onlyBy(owner) public{
	...
	function changeAccessPermissionStateByDelegation(address _delegator, address _delegatee) onlyBy(actionContract) public{
	...
	function revokeDelegation(address _delegatee) public{
	...
	function breakTheGlass(address _ad) onlyBy(actionContract) public returns(bool){
		if (blacklist[_ad] )
			revert();
	...
	function accessData() constant public returns(bool){
