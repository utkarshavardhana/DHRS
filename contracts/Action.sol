...
contract Action{
	...
	function changeAdministrator(address _add) onlyBy(root) public{
	...
	function addUser(address _ad, address _contract) ActionRequirement("addUser") public{
	...
	function addStaff(address _ad, address _contract) ActionRequirement("addStaff") public{
	...
	function roleAssign(address _ad, bytes32 _role) ActionRequirement("roleAssign") public{
		if (staffs[_ad] != address(0x0)){
	...
	function delegate_administrative(address _delegatee, bytes32[] _delepermi, bytes32[] _nondelepermi) ActionRequirement("delegation") public returns(bool){
	...
	function delegate_access(address _user, address _delegatee) ActionRequirement("delegation") public{
	...
	function breakTheGlass(address _ad, bytes32 hash, uint8 v, bytes32 r, bytes32 s) ActionRequirement("breakTheGlass") public Verify(hash, v, r, s) {
