...
contract Staff is User{
	...
	function Staff(address _actionContract, address _adminContract) User(_actionContract, _adminContract) public{
	...
	function changeRoleState(bytes32 _role) onlyBy(actionContract) public returns(bool){
		if (roles[_role])
			roles[_role] = false;
		else{
			roles[_role] = true;
	...
	function PermissionState(bytes32 _action) constant public returns(uint){
		uint current = 0;
		uint temp = 0;
	...
	function canDelegate() public view returns(bool){
		return (numDelegation < maxDelegation);
	}

	//The action contract can add a delegation
	function addDelegation(address _from, address _fromContract, address _toContract, uint _toIndex, bytes32[] _deleadmin, bytes32[] _nondeleadmin) onlyBy(actionContract) public returns(uint){
	...
	function revokeDelegation(uint _delegation, bytes32 _permi) public {
